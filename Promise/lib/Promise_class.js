...
static all = function (promises) {
  ...
  // 用来计算所有保存成功promise的数量
  let resolvedCount=0;
  // 返回一个新的promise
  return new Promise((resolve,reject)=>{
    //遍历promisess获取每一个promise的结果
    promises.forEach((p,index)=>{
      Promise.resolve(p).then(
        value=>{
          resolvedCount ++ //成功的数量加1
          values[index]=value //p成功，将成功的value保存values
          //如果全部成功，将成功的promise改为成功
...
promises.forEach((p,index)=>{
 Promise.resolve(p).then(
    value=>{
        resolve(value)//一旦有成功，将return的结果变为成功
    },
    reason=>{
      reject(reason)//只要一个失败了，return的promise就失败
    }
  )
})
